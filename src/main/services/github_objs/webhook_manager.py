...
from main.services.github_objs.webhook_creation_protocol import WebhookCreationProtocol

_WEBHOOK_URL = 'https://www.rehttp.net/p/https://revive-code-bot.ilaletdinov.ru/hook/github'


def hook_already_exists(gh_repo: Repository, url: str) -> bool:
...
class WebhookCreation(WebhookCreationProtocol):
    ...
    def create_if_needed(self, repo_db_record: GhRepo) -> None:
        ...
        if repo_db_record.has_webhook:
            return
        if hook_already_exists(self._gh_repo, _WEBHOOK_URL):
